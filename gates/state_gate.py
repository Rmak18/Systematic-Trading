...
def estimate_ns_factors(yields_df: pd.DataFrame, lam: float = 0.0609) -> pd.DataFrame:
    ...
    cols = [c for c in yields_df.columns if c in maturity_map]
    if len(cols) < 3:
        raise ValueError("Need at least 3 distinct maturities to estimate NS factors. Found: %s" % cols)

    taus = np.array([maturity_map[c] for c in cols], dtype=float)
    X = _ns_loadings(taus, lam=lam)

    betas = []
    for _, row in yields_df[cols].iterrows():
        y = row.values.astype(float)
        b, *_ = np.linalg.lstsq(X, y, rcond=None)
        betas.append(b)

    betas = np.asarray(betas)
    out = pd.DataFrame(betas, columns=["NS_level", "NS_slope", "NS_curvature"],
...
class StateGateTree:
    ...
    def _best_split_for_node(self, node: 'TreeNode',
        ...
        for feat in self.features_:
            ...
            try:
                qs = np.unique(np.quantile(x_ok, self.candidate_quantiles))
            except Exception:
                continue
            for thr in qs:
                left_mask = x_ok <= thr
...
def run_demo(yields_csv: str,
    ...
    # Calculate regime-specific volatilities and scaling factors
    regime_scales = {}
    regime_vols = {}
    ...
    for regime_id in sorted(regimes['Regime'].unique()):
        ...
        # Calculate volatility of NS factors within this regime
        if len(regime_factors) > 2:
            # Use standard deviation of factor changes as volatility measure
            factor_changes = regime_factors.diff().dropna()
            vol = np.sqrt(np.mean(factor_changes.std() ** 2))  # RMS of factor volatilities
        else:
            vol = 1.0
    ...
    regimes_out['Scale'] = regimes_out['Regime'].map(regime_scales)
    ...
    print(f"Tree structure: {json.dumps(tree_dict, indent=2)}")

    print(f"\nRegime distribution:")
    for regime_id in sorted(regimes_out['Regime'].unique()):
        count = (regimes_out['Regime'] == regime_id).sum()
        pct = count / len(regimes_out) * 100
        scale = regime_scales[regime_id]
        vol = regime_vols[regime_id]
        print(f"  Regime {regime_id}: {count} months ({pct:.1f}%) | Vol: {vol:.4f} | Scale: {scale:.3f}")
    print("="*60)
...
def cli():
    ...
    parser.add_argument("--yields_csv", required=True, help="CSV with Date and maturities (e.g., 3M,2Y,10Y...).")
